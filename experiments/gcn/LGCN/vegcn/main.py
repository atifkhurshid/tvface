...
def parse_args():
    ...
    parser.add_argument('--prefix', default=None, help='path to dataset (required if not set in config)')
...
def main():
    ...
    # append prefix from args to filepaths in cfg, if prefix not set in cfg
    if cfg.prefix == '':
        if args.prefix is not None:
            cfg.prefix = args.prefix
        else:
            print('Warning: \'prefix\' not set in config or arguments. It will be set to current directory.')
            cfg.prefix = './'
        for data in ['train_data', 'test_data']:
            if not hasattr(cfg, data):
                continue
            cfg[data]['feat_path'] = os.path.join(cfg.prefix, cfg[data]['feat_path'])
            cfg[data]['label_path'] = os.path.join(cfg.prefix, cfg[data]['label_path'])
            cfg[data]['knn_graph_path'] = os.path.join(cfg.prefix, cfg[data]['knn_graph_path'])
