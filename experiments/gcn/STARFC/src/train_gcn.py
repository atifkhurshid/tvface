...
def train_gcn(model, cfg, logger):
    ...
    if cfg.load_model:
        ...
        model_path = os.path.join(models_dir, backbone_name)
        head_path = os.path.join(models_dir, HEAD_name)
        print('loading model from {}'.format(model_path))
        model.load_state_dict(torch.load(model_path))
        print('loading head from {}'.format(head_path))
        HEAD1.load_state_dict(torch.load(head_path), False)
    """
    #load parameters from the pretrained model
    if cfg.load_model:
        model_path = os.path.join(cfg.model_root, cfg.backbone_name)
        head_path = os.path.join(cfg.model_root, cfg.head_name)
        print('loading model from {}'.format(model_path))
        model.load_state_dict(torch.load(model_path))
        print('loading head from {}'.format(head_path))
        HEAD1.load_state_dict(torch.load(head_path), False)
    ...
    for epoch in range(cfg.total_epochs):
        ...
        sims, cluster_id = index.search(center_fea, k=min(cfg.cluster_num+200, cls_num))  # search for the k-10 neighbor
        ...
        #for batch in range(20):
            #0.select ids
            sample_cluster_id = random.sample(list(cluster_id[batch]), min(cfg.cluster_num, cls_num))
            ...
            knn_prefix = os.path.join(cfg.work_dir, "rebuild_knn")
            ...
            if batch%10==0:
                print('{}, epoch:{}/{}, batch:{}/{}, loss:{}'.format(
                    datetime.datetime.now(), epoch + 1, cfg.total_epochs, batch, cls_num, loss), flush=True)

            if (batch+1)%100==0:
                if not os.path.exists(MODEL_ROOT):
                    os.makedirs(MODEL_ROOT)
                print('save model in epoch:{} batch:{} to {}'.format(epoch + 1, batch, MODEL_ROOT))
            ...
            if (batch + 1) % 300 == 0:
                ...
                print('the avg testing acc in epoch:{} batch:{} is :'.format(epoch + 1,batch), avg_acc)
                model.train()
                HEAD1.train()
